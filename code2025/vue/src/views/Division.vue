  ...
  <div>
    ...
    <div class="card" style="margin-bottom: 5px">
      <el-table :data="data.tableData" style="width: 100%" :header-cell-style="{ color: '#333', backgroundColor: '#eaf4ff' }">
        ...
        <el-table-column prop="collegeTitle" label="院系" />
    ...
    <el-dialog title="分类信息" v-model="data.formVisible" width="30%" destroy-on-close>
      <el-form ref="formRef" :model="data.form" :rules="data.rules" label-width="80px" style="padding: 20px 30px 10px 0">
        <el-form-item prop="title" label="班级">
          <el-input v-model="data.form.title" autocomplete="off" placeholder="请输入班级" />
        </el-form-item>
        <el-form-item prop="collegeTitle" label="所属院系">
          <el-select v-model="data.form.collegeId" placeholder="请选择院系" style="width: 100%">
            <el-option
...
const load = () => {
  request.get('/division/selectPage', {
    params: {
...
// 获取院校名称
const getCollegeTitle = (id) => {
  const found = data.collegeData.find(item => item.id === id)
...
const add = () => {
  request.post('/division/add', data.form).then(res => {
    if (res.code === '200') {
...
const update = () => {
  request.put('/division/update', data.form).then(res => {
    if (res.code === '200') {
...
const del = (id) => {
  ElMessageBox.confirm('删除后无法恢复，您确认删除吗？', '删除确认', { type: 'warning' }).then(() => {
    request.delete('/division/delete/' + id).then(res => {
      if (res.code === '200') {
