...
<!--  <div class="card" style="margin-bottom: 5px" v-if="data.user.role === 'USER'">-->
  ...
  <div class="card" style="margin-bottom: 5px">
    <el-table :data="data.tableData" style="width: 100%" :header-cell-style="{ color: '#333', backgroundColor: '#eaf4ff' }">
      ...
      <el-table-column prop="teacherName" label="作者" />
      <el-table-column prop="publish" label="书籍出版社" />
      <el-table-column prop="collegeTitle" label="所属院系" />
      <el-table-column prop="content" label="书籍简介">
  ...
  <el-dialog title="书籍出版信息" v-model="data.formVisible" width="60%" destroy-on-close>
    <el-form ref="formRef" :model="data.form" label-width="80px" style="padding: 20px 30px 10px 0">
      <el-form-item prop="file" label="书籍">
        <el-upload
      ...
      <el-form-item prop="title" label="书名">
        <el-input v-model="data.form.title" autocomplete="off" placeholder="请输入书名" />
      </el-form-item>
      ...
      <el-form-item prop="publish" label="出版社">
        <el-input v-model="data.form.publish" autocomplete="off" placeholder="请输入出版社" />
      </el-form-item>
      <el-form-item prop="content" label="书籍简介">
...
const viewContent = (content) => {
  ElMessageBox.alert(content, '书籍简介', { dangerouslyUseHTMLString: true })
}
