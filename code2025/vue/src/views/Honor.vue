...
<!--  <div class="card" style="margin-bottom: 5px" v-if="data.user.role === 'USER'">-->
    ...
    <el-table :data="data.tableData" style="width: 100%" :header-cell-style="{ color: '#333', backgroundColor: '#eaf4ff' }">
      <el-table-column label="攻略主图" width="100">
        <template #default="scope">
          <el-image v-if="scope.row.file" :src="scope.row.file" :preview-src-list="[scope.row.file]" :preview-teleported="true"
                    style="width: 50px; height: 50px; border-radius: 5px; display: block" />
        </template>
      </el-table-column>
      <el-table-column prop="title" label="荣誉证书名称" />
      <el-table-column prop="subjectTitle" label="学科方向" />
      <el-table-column prop="organizer" label="颁发组织" />
      <el-table-column prop="level" label="荣誉等级" />
      <el-table-column prop="teacherName" label="荣誉获得者" />
  ...
  <el-dialog title="荣誉信息" v-model="data.formVisible" width="60%" destroy-on-close>
    <el-form ref="formRef" :model="data.form" label-width="80px" style="padding: 20px 30px 10px 0">
      <el-form-item prop="file" label="荣誉证书">
        <el-upload
      ...
      <el-form-item prop="title" label="荣誉名称">
        <el-input v-model="data.form.title" autocomplete="off" placeholder="请输入荣誉名称" />
      </el-form-item>
      <el-form-item prop="organizer" label="荣誉名称">
        <el-input v-model="data.form.organizer" autocomplete="off" placeholder="请输入荣誉名称" />
      </el-form-item>
      <el-form-item prop="level" label="荣誉等级">
        <el-input v-model="data.form.level" autocomplete="off" placeholder="请输入荣誉名称" />
      </el-form-item>
...
const add = () => {
  request.post('/honor/add', data.form).then(res => {
    if (res.code === '200') {
...
const update = () => {
  request.put('/honor/update', data.form).then(res => {
    if (res.code === '200') {
...
const del = (id) => {
  ElMessageBox.confirm('删除后无法恢复，您确认删除吗？', '删除确认', { type: 'warning' }).then(() => {
    request.delete('/honor/delete/' + id).then(res => {
      if (res.code === '200') {
