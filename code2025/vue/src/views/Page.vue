...
<!--  <div class="card" style="margin-bottom: 5px" v-if="data.user.role === 'USER'">-->
  ...
  <div class="card" style="margin-bottom: 5px">
    <el-table :data="data.tableData" style="width: 100%" :header-cell-style="{ color: '#333', backgroundColor: '#eaf4ff' }">
      ...
      <el-table-column prop="subjectTitle" label="论文方向" />
      <el-table-column prop="division" label="论文分区" />
  ...
  <el-dialog title="科研项目信息" v-model="data.formVisible" width="60%" destroy-on-close>
    <el-form ref="formRef" :model="data.form" label-width="80px" style="padding: 20px 30px 10px 0">
      ...
      <el-form-item prop="title" label="论文方向">
        <el-select
      ...
      <el-form-item prop="division" label="论文分区">
        <el-input v-model="data.form.division" autocomplete="off" placeholder="请输入论文分区" />
      </el-form-item>
...
const add = () => {
  request.post('/page/add', data.form).then(res => {
    if (res.code === '200') {
...
const update = () => {
  request.put('/page/update', data.form).then(res => {
    if (res.code === '200') {
...
const del = (id) => {
  ElMessageBox.confirm('删除后无法恢复，您确认删除吗？', '删除确认', { type: 'warning' }).then(() => {
    request.delete('/page/delete/' + id).then(res => {
      if (res.code === '200') {
